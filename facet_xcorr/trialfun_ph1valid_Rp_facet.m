function [trl, event] = trialfun_ph1valid_Rp_facet(cfg)

%% the first part is common to all trial functions
% read the header (needed for the samping rate) and the events
hdr        = ft_read_header(cfg.dataset, 'headerformat', 'facet_txt');
event      = ft_read_event(cfg.dataset, 'eventformat', 'facet_txt');
