function [ ga, fig ] = ph1valid03_timelock (experiment, forceInd, forceGa)
%% Timelock
...
function [ TlCond ] = calcTimelock (experiment, subjid, emgPreproDir, force)


 target_file = fullfile(emgPreproDir, subjid, [subjid '_timelock_' experiment '.mat']);
 ...
 prepro_file = fullfile(emgPreproDir, subjid, [subjid '_class_' experiment '.mat']);
 load(prepro_file);
 ...
 save(fullfile(emgPreproDir, subjid, [subjid '_timelock_' experiment '.mat']), 'TlCond');

 function [ ga, TlCond ] = aggregateTimelocks (experiment, SessionInfo, force, forceGA)

ga_file = fullfile(SessionInfo.outDir, ['tlga_' experiment '.mat']);
...
for i = 1:46
    ...
    arg = ['VP' b];
    try
        TlCond(i) = calcTimelock (experiment, arg, SessionInfo.emgPreproDir, force);
    catch ME
        disp(ME);
