function [trl, event] = trialfun_ph1valid_Ts(cfg)
...
ts_trig = [ 241 242 251:253 141 142 151:153 ];
for i=1:numel(event)
    sel(i) = sel(i) && ismatch(event(i).type, 'STATUS');
end
for i=1:numel(event)
    sel(i) = sel(i) && ismatch(event(i).value, ts_trig);
end
sel = find(sel);
...
if length(trl) < 350
  ...
  %  end;
elseif length(trl) > 370
    nTrl = length(trl);
    trl = trl(length(trl) - 360 + 1:end,:);
